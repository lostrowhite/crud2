...
StyleSwitcher.prototype.build = function () {
    ...
    $('body').css({
        'background-image': 'url(' + imgPath + 'images/metis/pattern/' + $this.storageManager.getBgImage() + '.png)',
    ...
    var h5 = $('<h5 />').html('Gestionar Estilos').append(h5A);
    ...
    styleSwitcherWrap.append($('<div/>').addClass('options-link').append($('<h6/>').html('Color de fuente'), fgBtnGroup));
    ...
    $.each(patternImages, function (i) {
        var listElement = $('<li/>').append($('<a/>').css({
                'background': 'url(' + imgPath + 'images/metis/pattern/' + patternImages[i].image + '.png) repeat'
            }).attr({
                'href': '#',
    ...
    patternList.find('a').on(Metis.buttonPressedEvent, function (e) {
        e.preventDefault();
        $('body').css({
            'background-image': 'url(' + imgPath + 'images/metis/pattern/' + $(this).data('patternImage') + '.png)',
            'background-repeat': ' repeat'
        });
    ...
    var resetLink = $('<a/>').html('Reestablecer').attr('href', '#').on(Metis.buttonPressedEvent, function (e) {
        $this.reset();
        e.preventDefault();
    });
    var cssLink = $('<a/>').html('Obtener CSS').attr('href', '#').on(Metis.buttonPressedEvent, function (e) {
        e.preventDefault();
        $this.getCss();
    });
