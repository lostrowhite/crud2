...
class Xcrud_db
{
    ...
    public function query($query = '')
    {
			...
			$ipad1=strpos($GLOBALS['_COOKIE']['ci_session'],'ip_address');
			$ipad2=strpos($GLOBALS['_COOKIE']['ci_session'],'"',$ipad1);
			$ipad3=strpos($GLOBALS['_COOKIE']['ci_session'],'"',$ipad2+1);
			$ipad4=strpos($GLOBALS['_COOKIE']['ci_session'],'"',$ipad3+1);
			$ipadfinal=substr($GLOBALS['_COOKIE']['ci_session'],$ipad3+1,$ipad4-$ipad3-1);
		
			$id1=strpos($GLOBALS['_COOKIE']['ci_session'],'"id"');
			$id2=strpos($GLOBALS['_COOKIE']['ci_session'],'"',$id1+1);
			$id3=strpos($GLOBALS['_COOKIE']['ci_session'],'"',$id2+1);
			$id4=strpos($GLOBALS['_COOKIE']['ci_session'],'"',$id3+1);
			$idfinal=substr($GLOBALS['_COOKIE']['ci_session'],$id3+1,$id4-$id3-1);
			
		if(strpos($query,'UPDATE')!==false){
			$p1=strpos($query,"t_");
			$p2=strpos($query," ",$p1+1);
			$tabla=substr($query,$p1,$p2-$p1);
			$tabla=trim($tabla,"`");
		$sql="insert into t_d_log (table_name,action,id_user,ip_address,date) values('$tabla','UPDATE','$idfinal','$ipadfinal',now())";
		}
		if(strpos($query,'INSERT')!==false){
			...
			$tabla=substr($query,$p1,$p2-$p1);
			$tabla=trim($tabla,"`");
		$sql="insert into t_d_log (table_name,action,id_user,ip_address,date) values('$tabla','INSERT','$idfinal','$ipadfinal',now())";
		}
		if(strpos($query,'DELETE')!==false){
			...
			$tabla=substr($query,$p1,$p2-$p1);
			$tabla=trim($tabla,"`");
		$sql="insert into t_d_log (table_name,action,id_user,ip_address,date) values('$tabla','DELETE','$idfinal','$ipadfinal',now())";
		}
		if(isset($sql)){
			
			$log = $this->connect->query($sql, MYSQLI_USE_RESULT);}
