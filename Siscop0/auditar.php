...
function Footer()
{
	...
	$this->Cell(100,10,'REPORTE DE AUDITORIA:'.date('d/m/Y h:m:s').'',0,0,'L');
	...
	$pdf->SetWidths(array(50, 30, 35, 110));
    ...
    $pdf->SetTextColor(255);
		...
		for($i=0;$i<1;$i++)
			{
				$pdf->Row(array('FECHA', 'IP', 'USUARIO', 'EVENTO'));
			}
	
	$historial = $con->conectar();	
	$strConsulta = "SELECT * FROM auditoria";
	...
	for ($i=0; $i<$numfilas; $i++)
		{
			...
			$pdf->SetFillColor(184,184,184);
    			$pdf->SetTextColor(0);
				$pdf->Row(array(date("d-m-Y H:m:s",strtotime($fila['fecha'])), $fila['ip'], $fila['usuario'], $fila['detalle']));
