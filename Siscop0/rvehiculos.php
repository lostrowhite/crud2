...
function Footer()
{
	...
	$this->Cell(100,10,'REPORTE DE VEHÍCULOS :'.date('d/m/Y h:m:s').'',0,0,'L');
	...
	$pdf->SetWidths(array(20, 60, 35, 20, 35, 35, 35));
    ...
    $pdf->SetTextColor(255);
		...
		for($i=0;$i<1;$i++)
			{
				$pdf->Row(array('NSOCIO', 'NOMBRE', 'CÉDULA', 'PLACA', 'MARCA', 'MODELO','ESTADO'));
			}
	
	$historial = $con->conectar();	
	$strConsulta = "SELECT * FROM vehiculos ORDER BY estado";
	...
	for ($i=0; $i<$numfilas; $i++)
		...
		break;
			...
			if($i%2 == 1)
			{
    			...
    			$pdf->SetTextColor(0);
				$pdf->Row(array($cliente['nsocio'], $cliente['nombre'], $cliente['cedula'],$fila['placa'], $fila['marca'], $fila['modelo'],$estado));
			}
			else
			{
